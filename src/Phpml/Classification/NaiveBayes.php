...
class NaiveBayes implements Classifier
{
    ...
    protected function predictSampleProb(array $sample) {
        $predictions = [];
        foreach ($this->labels as $label) {
            $p = $this->p[$label];
            for ($i = 0; $i < $this->featureCount; ++$i) {
        ...
        foreach($predictions as $key => $value)
            $predictions[$key] = ($value / $total) * 100;
        arsort($predictions, SORT_NUMERIC);
        return $predictions;
    }
     ...
     * @return mixed
     */
    public function predictIt(array $samples)
    {
        if (!$predicted = $this->predict($samples)) {
            $probs = $this->predictProb($samples);
            array_shift($probs);
            $prob = reset($probs);
            if ($prob > 13.4) {
            //if ($prob > 12) {
     ...
     * @return mixed
     */
    public function predictProb(array $samples)
    {
        if (!is_array($samples[0])) {
            return $this->predictSampleProb($samples);
        }

        $predicted = [];
        foreach ($samples as $index => $sample) {
            $predicted[$index] = $this->predictSampleProb($sample);
        }
